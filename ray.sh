...
nft add set inet filter knock_fail '{ type ipv4_addr; flags dynamic; timeout 24h; size 65536; gc-interval 5m; }'
...
nft add set inet filter knock_fail_v6 '{ type ipv6_addr; flags dynamic; timeout 24h; size 65536; gc-interval 5m; }'
...
nft add rule inet filter input ip saddr @xray_clients tcp dport { $PORT_VLESS, $PORT_TROJAN, $SSH_PORT } counter update @xray_clients '{ ip saddr }' accept
nft add rule inet filter input ip saddr @xray_clients udp dport $PORT_HYSTERIA counter update @xray_clients '{ ip saddr }' accept
nft add rule inet filter input ip6 saddr @xray_clients_v6 tcp dport { $PORT_VLESS, $PORT_TROJAN, $SSH_PORT } counter update @xray_clients_v6 '{ ip6 saddr }' accept
...
nft add rule inet filter input tcp dport $K1 limit rate 3/minute burst 3 packets jump knock_stage1_handler
nft add rule inet filter input udp dport $K1 limit rate 3/minute burst 3 packets jump knock_stage1_handler
nft add rule inet filter input tcp dport $K1 add @knock_fail '{ ip saddr }' log prefix "nft-knock-fail-v4: " drop
nft add rule inet filter input udp dport $K1 add @knock_fail '{ ip saddr }' log prefix "nft-knock-fail-v4: " drop
nft add rule inet filter input tcp dport $K1 add @knock_fail_v6 '{ ip6 saddr }' log prefix "nft-knock-fail-v6: " drop
nft add rule inet filter input udp dport $K1 add @knock_fail_v6 '{ ip6 saddr }' log prefix "nft-knock-fail-v6: " drop
nft add rule inet filter input tcp dport $K2 ip saddr @knock_stage1 limit rate 3/minute burst 3 packets jump knock_stage2_handler
nft add rule inet filter input udp dport $K2 ip saddr @knock_stage1 limit rate 3/minute burst 3 packets jump knock_stage2_handler
nft add rule inet filter input tcp dport $K2 ip6 saddr @knock_stage1_v6 limit rate 3/minute burst 3 packets jump knock_stage2_handler
nft add rule inet filter input udp dport $K2 ip6 saddr @knock_stage1_v6 limit rate 3/minute burst 3 packets jump knock_stage2_handler
nft add rule inet filter input tcp dport $K2 add @knock_fail '{ ip saddr }' log prefix "nft-knock-fail-v4: " drop
nft add rule inet filter input udp dport $K2 add @knock_fail '{ ip saddr }' log prefix "nft-knock-fail-v4: " drop
nft add rule inet filter input tcp dport $K2 add @knock_fail_v6 '{ ip6 saddr }' log prefix "nft-knock-fail-v6: " drop
nft add rule inet filter input udp dport $K2 add @knock_fail_v6 '{ ip6 saddr }' log prefix "nft-knock-fail-v6: " drop
nft add rule inet filter input tcp dport $K2 drop
nft add rule inet filter input udp dport $K2 drop
nft add rule inet filter input tcp dport $K3 ip saddr @knock_stage2 limit rate 3/minute burst 3 packets jump knock
nft add rule inet filter input udp dport $K3 ip saddr @knock_stage2 limit rate 3/minute burst 3 packets jump knock
nft add rule inet filter input tcp dport $K3 ip6 saddr @knock_stage2_v6 limit rate 3/minute burst 3 packets jump knock
nft add rule inet filter input udp dport $K3 ip6 saddr @knock_stage2_v6 limit rate 3/minute burst 3 packets jump knock
nft add rule inet filter input tcp dport $K3 add @knock_fail '{ ip saddr }' log prefix "nft-knock-fail-v4: " drop
nft add rule inet filter input udp dport $K3 add @knock_fail '{ ip saddr }' log prefix "nft-knock-fail-v4: " drop
nft add rule inet filter input tcp dport $K3 add @knock_fail_v6 '{ ip6 saddr }' log prefix "nft-knock-fail-v6: " drop
nft add rule inet filter input udp dport $K3 add @knock_fail_v6 '{ ip6 saddr }' log prefix "nft-knock-fail-v6: " drop
nft add rule inet filter input tcp dport $K3 drop
nft add rule inet filter input udp dport $K3 drop
...
nft add rule inet filter output icmp type { echo-request, echo-reply } accept
...
if [[ -n $IFACE ]]; then
    ...
    nft add chain ip6 nat postrouting '{ type nat hook postrouting priority 100; policy accept; }' 2>/dev/null || true
    nft add rule ip6 nat postrouting oifname "$IFACE" masquerade
fi
EOF
